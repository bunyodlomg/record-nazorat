    ...
    <motion.div variants={listItem}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:12, flexWrap:'wrap' }}>
        <button onClick={() => !noTeacher && onOpenTeacher?.(teacher._id)}
          ...
          onMouseEnter={!CAN_HOVER ? undefined : e => { if (!noTeacher) { e.currentTarget.style.background='var(--bg-hover)'; e.currentTarget.style.borderColor='rgba(45,212,191,0.30)'; } }}
          onMouseLeave={!CAN_HOVER ? undefined : e => { if (!noTeacher) { e.currentTarget.style.background='var(--bg-subtle)'; e.currentTarget.style.borderColor='var(--border)'; } }}>
          {noTeacher
      ...
      <div className="groups-grid">
        {groups.map(g => (
          <GroupCard key={g._id} g={g} onOpenTeacher={onOpenTeacher} onOpenGroup={onOpenGroup}
            onEdit={isAdmin ? onEdit : undefined} onRemove={isAdmin ? onRemove : undefined}/>
        ))}
      </div>
    </motion.div>
  );
      ...
      <AnimatePresence mode="wait">
        {!loading && !error && groups.length > 0 && groupBy === 'group' && (
          <motion.div key="by-group" className="groups-grid"
            ...
            {groups.map(g => (
              <GroupCard key={g._id} g={g} onOpenTeacher={onOpenTeacher} onOpenGroup={onOpenGroup}
                onEdit={isAdmin ? openEdit : undefined} onRemove={isAdmin ? remove : undefined}/>
            ))}
          </motion.div>
        )}
